...
2. **Infrastructure Layer:** Generate GORM Models inside `internal/<domain>/infrastructure/models/` (package `models`) and implement the Postgres Repository in `internal/<domain>/infrastructure/postgres.go` (package `infrastructure`). Insert with `Create()`, never `Save()` (persistence-convention.md §1).
