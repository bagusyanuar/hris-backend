...
0. **Scope classification (BEFORE Domain Layer):** classify every entity per [scoping-convention.md](../rules/scoping-convention.md) §1 (Company-owned default → `company_id`; Company+Location → `+branch_id`; Global master → none, justified). The chosen scope columns MUST appear consistently in entity, GORM model, DTO, and match the DBML. `FindXxx`/`ListXxx` must be scope-aware (`scope.FromContext(ctx)` filter). Columns + signatures are written now even though `companies`/`branches` FK and RBAC enforcement are staged (scoping-convention.md §4).
1. **Domain Layer:** Generate `entity.go` and `repository.go` (interfaces) inside `internal/<domain>/domain/` (package `domain`). UUID generated inside the constructor only; not-found is a sentinel error, never `(nil, nil)`. Constructor takes `companyID` (+ `branchID` if location-bound) as required scope params.
