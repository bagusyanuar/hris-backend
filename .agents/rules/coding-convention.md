...
2. **Error Handling**:
   ...
   - **Jangan bocorkan error internal ke client**: pada response `5xx`, DILARANG mengirim `err.Error()` mentah (bisa membocorkan SQL/nama kolom/driver). Log error lengkap di server, balikan pesan generik (`"internal server error"`). `err.Error()` hanya untuk sentinel domain error yang aman dibaca user. Lihat [persistence-convention.md](persistence-convention.md) §4.
