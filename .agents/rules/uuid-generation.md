...
UUID **hanya boleh** di-generate di satu tempat: **Domain Layer (constructor)**, dengan `BeforeCreate` sebagai jaring pengaman di Infrastructure. **Application Layer DILARANG** meng-generate UUID sendiri lalu mengopernya ke constructor. Terapkan pola ini **seragam di semua domain** (jangan Employee generate di domain sementara Organization generate di service).
