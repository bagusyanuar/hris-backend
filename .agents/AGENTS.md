...
## Aturan Utama:
1. **Domain-Driven Design (DDD)**: Patuhi batasan layer (`domain`, `application`, `infrastructure`, `interfaces`).
2. **Tanpa CQRS**: Gunakan application service terpadu di `application/` (`service.go` untuk read & write logic), jangan memisahkannya menjadi command & query.
