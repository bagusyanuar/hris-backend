...
1. [Architecture & Struktur Folder (DDD)](rules/architecture.md) — struktur folder, dependency rules, aturan coding per layer (Domain/Application/Adapter/Interfaces).
...
9. [Konvensi Scoping Multi-Entity (Company & Branch)](rules/scoping-convention.md) — aksioma multi-PT/multi-cabang, klasifikasi kelas scope entity, kolom `company_id`/`branch_id` non-nullable, filter scope di query boundary via `scope.FromContext`, enforcement bertahap (staged sampai Organization & RBAC landing).
10. [Konvensi Pagination, Sort, Order & Search](rules/pagination-convention.md) — `pkg/pagination` wajib untuk semua endpoint List/FindAll, pembagian tanggung jawab per layer (domain tetap primitif, adapter wajib whitelist `SortMap`/`OrderClause` & `SearchClause` untuk cegah SQL injection lewat `Order()`/`Where()`). `search` jadi query param standar kalau entity punya kolom text.
11. [Konvensi Preload Relasi di Response DTO](rules/preload-convention.md) — nama entity FK (many-to-one) WAJIB nested `{id, name}` di response, bukan flat `<relasi>_name`; resolusi lewat batch lookup (`FindNamesByIDs`) di Application Layer, cegah N+1.
